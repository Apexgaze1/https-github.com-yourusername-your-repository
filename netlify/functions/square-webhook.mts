...
export default async (req:Request) => {
 ...
 if(!eventId) return Response.json({accepted:false,error:"missing_event_id"},{status:400});
 const claimed=await claimEvent(eventId,{type:event?.type||null});
 if(claimed.duplicate) return Response.json({accepted:true,duplicate:true,eventId});
 return Response.json({accepted:true,duplicate:false,eventId,type:event?.type||null,mode:"validated-durable-ingress",fulfillment:"held_until_order_mapping_is_proven"});
};
